package io.github.mcpaimon.extension.player.foliamc;
...
/**
 * The PlayerTools extension class for the MCAgents system on FoliaMC.
...
public class PlayerTools implements IMCExtension {
    ...
    public void onLoad(Object plugin, Executor executor) {
        if (plugin instanceof MCAgentsPlugin mcagentsPlugin) {
            ...
            // Folia has no main thread; run player affecting tasks on the entity scheduler
            PlayerTaskScheduler scheduler =
                    (target, task) -> target.getScheduler().run(mcagentsPlugin, scheduledTask -> task.run(), null);
            ...
            mcagentsPlugin.getLogger().info("PlayerTools extension loaded successfully on FoliaMC. Tools registered.");
    ...
    public void onDisable(Object plugin, Executor executor) {
        if (plugin instanceof JavaPlugin javaPlugin) {
            javaPlugin.getLogger().info("PlayerTools extension has been disabled on FoliaMC.");
        }
    }
}
